...
function renderCombinations() {
    ...
    const variantsText = combos.length === 1 ? 'вариант' : (combos.length < 5 ? 'варианта' : 'вариантов');
    let filterInfo = '';
    if (selectedDigits.length > 0 && originalCount > combos.length) {
        filterInfo = `  (отфильтровано ${originalCount - combos.length})`;
    }
    comboListDiv.innerHTML = html + `<div style="flex-basis:100%; text-align:center; font-size:0.7rem; margin-top:12px; color:#6c85a3;">${combos.length} ${variantsText}${filterInfo}</div>`;
}
